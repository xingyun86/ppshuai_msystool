...
int main(int argc, char ** argv)
{
	...
	std::string strFileM = strPath + "\\1.0-M\\etc\\fstab";
	std::string strFile32 = strPath + "\\1.0-32\\etc\\fstab";
	std::string strFile64 = strPath + "\\1.0-64\\etc\\fstab";

	std::string strPathM = strPath + "/1.0-M/mingw /mingw";
	std::string strPath32 = strPath + "/1.0-32/TDM-GCC-32 /mingw";
	std::string strPath64 = strPath + "/1.0-64/TDM-GCC-64 /mingw";

	std::fstream fM(strFileM);
	if (fM.is_open())
	{
		std::cout << "Install msys default mingw 32..." << std::endl;
		fM.write(strPathM.c_str(), strPathM.length());
		fM.close();
		std::cout << "Install msys default mingw 32 complete!" << std::endl;
	}

	std::fstream f32(strFile32);
	if (f32.is_open())
	{
		std::cout << "Install msys tdm-gcc mingw 32..." << std::endl;
		f32.write(strPath32.c_str(), strPath32.length());
		f32.close();
		std::cout << "Install msys tdm-gcc mingw 32 complete!" << std::endl;
	}

	std::fstream f64(strFile64);
	if (f64.is_open())
	{
		std::cout << "Install msys tdm-gcc mingw 64..." << std::endl;
		f64.write(strPath64.c_str(), strPath64.length());
		f64.close();
		std::cout << "Install msys tdm-gcc mingw 64 complete!" << std::endl;
	}
